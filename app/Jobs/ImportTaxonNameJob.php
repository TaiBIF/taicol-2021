...
class ImportTaxonNameJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $service = new TaxonNameImportService($sheet);
            ...
            Log::error('ImportTaxonNameJob failed', [
                'log_id' => $this->logId,
                'error' => $e->getMessage(),
            ]);
            $log->update([
    ...
    /** 在原 Excel 每列加上 error_message 欄，存到 public/import/taxon-name 供下載 */
    private function writeErrorFile($sheet, $spreadsheet, array $errorRows, int $logId): string
    {
        ...
        $relative = 'import/taxon-name/error_' . $logId . '_' . time() . '.xlsx';
    ...
    public function failed(\Throwable $e): void
    {
        Mail::to(env('TAICOL_EMAIL', 'catalogueoflife.taiwan@gmail.com'))
            ->send(new Email(
                '學名 Excel 匯入任務最終失敗：<br>' . nl2br($e->getMessage()),
                'TaiCOL - 學名匯入失敗',
                'TaiCOL管理員'
            ));
    }
