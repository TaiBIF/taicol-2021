...
class ImportPersonJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $service = new PersonImportService($sheet);
            ...
            Log::error('ImportPersonJob failed', [
                'log_id' => $this->logId,
                'error' => $e->getMessage(),
            ]);
            $log->update([
    ...
    /** 在原 Excel 每列加上 error_message 欄，存到 public/import/person 供下載 */
    private function writeErrorFile($sheet, $spreadsheet, array $errorRows, int $logId): string
    {
        ...
        $relative = 'import/person/error_' . $logId . '_' . time() . '.xlsx';
