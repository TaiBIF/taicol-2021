<template>
    <div class="px-4 py-2 form">
        <div class="overflow-y-auto body flex">
            <!-- taxon view -->
            <div class="flex-1 p-4">
                <p class="mb-3 font-bold">學名檢視</p>
                <div v-for="referenceUsages in usagesGroupByReference">
                    ...
                    <div class="item-container draggable-container mb-5">
                        <div v-for="(item, index) in referenceUsages">
                            <div v-if="!item.content" class="border border-gray-100 px-4 py-1 mb-3 shadow-md flex cursor-pointer">
                                <p class="italic text-gray-400">此收藏已不存在</p>
                            </div>
                            <div v-else v-for="usage in item.content"
                            ...
                            >
                                ...
                                <div class="buttons bg-white bg-opacity-25 is-right" style="min-width: 100px">
                                    <router-link class="g-button is-small mr-2" :to="`/taxon-names/${usage.taxonName.id}`">
                ...
                <div class="item-container draggable-container">
                    <div v-for="item in taxonNames"
                        ...
                        <taxon-name-full-label v-else :taxon-name="item.content" :key="item.id" class="flex-1"
                                               :with-color="true"></taxon-name-full-label>
                        <div class="buttons bg-white bg-opacity-25 is-right" style="min-width: 100px">
                            <router-link class="g-button is-small mr-2" :to="`/taxon-names/${item.content.id}`">
                                <i class="fas fa-external-link-alt"></i>
                            </router-link>
            ...
            <!-- reference view -->
            <div class="flex-1 p-4">
                <p class="mb-3 font-bold">文獻檢視</p>
                <div v-for="g in usageGroupByTaxonName" class="mb-5">
                    <div v-if="g.length !== 0 && g[0].content[0].taxonNameId" class="mb-3">
                        <taxon-name-full-label :taxon-name="g[0].content[0].taxonName" :with-color="true"></taxon-name-full-label>
                    </div>
                    <div class="item-container draggable-container">
                        <div v-for="r in g">
                            <div class="border border-gray-100 px-4 py-1 mb-3 shadow-md flex cursor-pointer">
                                ...
                                <div class="buttons bg-white bg-opacity-25 is-right" style="min-width: 100px">
                                    <router-link class="g-button is-small mr-2" :to="`/references/${r.reference.id}`">
                                        <i class="fas fa-external-link-alt"></i>
                                    </router-link>
                ...
                <div class="item-container draggable-container">
                    <div v-for="r in references"
                        ...
                        <div class="buttons bg-white bg-opacity-25 is-right" style="min-width: 100px">
                            <router-link class="g-button is-small mr-2" :to="`/references/${r.id}`">
                                <i class="fas fa-external-link-alt"></i>
                            </router-link>
                            <a class="close-button is-small"
...
export default {
    ...
    computed: {
        taxonNames() {
            return (this.itemGroup[2] || []).sort((a, b) => {
                return (a.content?.name > b.content?.name) - (a.content?.name < b.content?.name);
            });
        },
        ...
        usageGroupByTaxonName() {
            return groupBy(
                ...
                function(u) { return u.content[0].taxonNameId }
            );
        },
        references() {
