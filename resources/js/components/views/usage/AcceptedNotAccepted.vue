...
export default {
    render(createElement) {
        ...
        if (this.taxonName) {
            this.taxonName.properties.isApprovedList = false;
        }
        ...
        const indicationGroupDOM = createElement(
            ...
            [
                ...this.interleave(this.indications.map((i) => createElement(
                    'span',
                    {
                        class: {
            ...
            this.interleave([
                ...
                createElement(
                    ...
                    this.interleave(
                        ...
                        this.taxonName.nomenclature.group === 'animal' ? ': ' : ', ',
                    ),
                ),
    ...
    computed: {
        refPreview() {
            ...
            if (this.taxonName.nomenclature?.group === 'plant') {
                ...
                return [
                    ...
                    comboPlant(this.perUsages.slice(1, this.perUsages.length), comboLast),
                ]
                    .filter(Boolean)
                    .join('; ');
            }

            if (this.taxonName.nomenclature?.group === 'bacteria') {
                const firstPerUsage = this.perUsages[0];
                // Type strain: [citation_note_number]
                return [
                    firstPerUsage ? comboPlant(
                        [firstPerUsage],
                        () => {
